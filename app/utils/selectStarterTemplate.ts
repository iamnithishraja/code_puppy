...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .puppy
  filteredFiles = filteredFiles.filter((x) => x.path.startsWith('.puppy') == false);

  // check for ignore file in .puppy folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.puppy') && x.name == 'ignore');
...
<puppyArtifact id="imported-files" title="${title || 'Importing Starter Files'}" type="bundled">
${filesToImport.files
  .map(
    (file) =>
      `<puppyAction type="file" filePath="${file.path}">
${file.content}
</puppyAction>`,
  )
  .join('\n')}
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.puppy')).find((x) => x.name == 'prompt');
