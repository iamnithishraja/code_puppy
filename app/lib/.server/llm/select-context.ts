...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifypuppyActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifypuppyActions(content);

      content = content.replace(/<div class=\\"__puppyThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`puppy failed to select files`);
  }
