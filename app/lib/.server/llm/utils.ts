...
export function simplifypuppyActions(input: string): string {
  // Using regex to match puppyAction tags that have type="file"
  const regex = /(<puppyAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/puppyAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<puppyAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</puppyAction>`;
    });

  return `<puppyArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</puppyArtifact>`;
}
