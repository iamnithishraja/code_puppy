...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out puppy artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <puppy', 'Foo bar '],
      ['Foo bar <puppya', 'Foo bar <puppya'],
      ['Foo bar <puppyA', 'Foo bar '],
      ['Foo bar <puppyArtifacs></puppyArtifact>', 'Foo bar <puppyArtifacs></puppyArtifact>'],
      ['Before <oltArtfiact>foo</puppyArtifact> After', 'Before <oltArtfiact>foo</puppyArtifact> After'],
      ['Before <puppyArtifactt>foo</puppyArtifact> After', 'Before <puppyArtifactt>foo</puppyArtifact> After'],
    ])('should correctly parse chunks and strip out puppy artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <puppyArtifact title="Some title" id="artifact_1">foo bar</puppyArtifact> Some more text',
        {
      ...
      [
        [
          'Some text before <puppyArti',
          'fact',
          ' title="Some title" id="artifact_1" type="bundled" >foo</puppyArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' ',
          '>',
          'foo</puppyArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' >fo',
          'o</puppyArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <puppyArtifact title="Some title" id="artifact_1">foo</puppyArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <puppyArtifact title="Some title" id="artifact_1"><puppyAction type="shell">npm install</puppyAction></puppyArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <puppyArtifact title="Some title" id="artifact_1"><puppyAction type="shell">npm install</puppyAction><puppyAction type="file" filePath="index.js">some content</puppyAction></puppyArtifact> After',
        {
          output: 'Before  After',
