...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          puppy_user_profile: this._safeGetItem('puppy_user_profile'),
          puppy_settings: this._safeGetItem('puppy_settings'),
          puppy_profile: this._safeGetItem('puppy_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('puppy_viewed_features'),
          developer_mode: this._safeGetItem('puppy_developer_mode'),
        ...
        ui: {
          // Tab configuration
          puppy_tab_configuration: this._safeGetItem('puppy_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('puppy_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('puppy_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          puppy_read_logs: this._safeGetItem('puppy_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('puppy_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('puppy_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.puppy_tab_configuration) {
        try {
          this._safeSetItem('puppy_tab_configuration', data.ui.puppy_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'puppy_acknowledged_debug_issues',
        'puppy_acknowledged_connection_issue',
        'error_logs',
        'puppy_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('puppy_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
